...
int Win32NetConfig::allowPort(int port)
{
    ...
    p.waitForFinished();
    test +=p.exitCode();
    command = "netsh advfirewall firewall add rule name=monitorP"+QString::number(port).toStdString()+" dir=OUT protocol=UDP localport="+QString::number(port).toStdString()+" action=allow";
    p.start(command.c_str());
    p.waitForFinished();
    test +=p.exitCode();
    command = "netsh advfirewall firewall add rule name=monitorP"+QString::number(port).toStdString()+" dir=OUT protocol=TCP localport="+QString::number(port).toStdString()+" action=allow";
    p.start(command.c_str());
    p.waitForFinished();
    test +=p.exitCode();
    return test;
...
int Win32NetConfig::blockPort(int port)
{
    ...
    p.waitForFinished();
    test +=p.exitCode();
    command = "netsh advfirewall firewall add rule name=monitorP"+QString::number(port).toStdString()+" dir=OUT protocol=UDP localport="+QString::number(port).toStdString()+" action=block";
    p.start(command.c_str());
    p.waitForFinished();
    test +=p.exitCode();
    command = "netsh advfirewall firewall add rule name=monitorP"+QString::number(port).toStdString()+" dir=OUT protocol=TCP localport="+QString::number(port).toStdString()+" action=block";
    p.start(command.c_str());
    p.waitForFinished();
    test +=p.exitCode();
    return test;
...
int Win32NetConfig::removeForward(std::string listenIP, int port_lower, int port_higher, std::string dstIP)
{
    ...
    for(int i = port_lower; i<=port_higher;i++){
    std::string command = "netsh interface portproxy add v4tov4 listenport="+QString::number(i).toStdString()+" listenaddress="+listenIP;
    p.start(command.c_str());
    p.waitForFinished();
    test+=p.exitCode();
    }
